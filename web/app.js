      ...
      ${sorted.map(([lang, count]) => {
        ...
        return `
          <div class="lang-bar-item">
            <span class="lang-bar-label">${esc(lang)}</span>
...
function renderSearchPreview() {
  ...
  preview.innerHTML = matches.map(r => {
    ...
    return `
      <div class="preview-item" data-repo="${esc(r.name)}">
        <div class="preview-icon">${icon}</div>
        <div class="preview-info">
          <span class="preview-name">${esc(r.name)}</span>
          <span class="preview-desc">${esc((r.description || '').substring(0, 40))}...</span>
        </div>
  ...
  document.getElementById('modalDesc').innerHTML = `
    <div class="compare-container">
      <div class="compare-repo">
        ...
        <h3>${esc(repo1.name.split('/')[1])}</h3>
        <p class="compare-author">${esc(repo1.name.split('/')[0])}</p>
        <p class="compare-desc">${esc(repo1.description || '-')}</p>
        ...
        <a href="${esc(repo1.url)}" target="_blank" rel="noopener noreferrer" class="compare-link">${t('openInGithub')}</a>
      </div>
      <div class="compare-vs">VS</div>
      <div class="compare-repo">
        <div class="compare-icon">${icon2}</div>
        <h3>${esc(repo2.name.split('/')[1])}</h3>
        <p class="compare-author">${esc(repo2.name.split('/')[0])}</p>
        <p class="compare-desc">${esc(repo2.description || '-')}</p>
        <div class="compare-stats">
          <div class="compare-stat">
            <span class="compare-label">Stars</span>
            <div class="compare-bar-wrap">
        ...
        <a href="${esc(repo2.url)}" target="_blank" rel="noopener noreferrer" class="compare-link">${t('openInGithub')}</a>
      </div>
    </div>
  `;
  document.getElementById('modalStats').innerHTML = '';
...
function showRecommendations() {
  ...
  document.getElementById('modalDesc').innerHTML = recommended.map((r, i) => {
    const icon = LANG_ICONS[r.language] || LANG_ICONS['Unknown'];
    return `
      <div class="recommend-item" data-repo="${esc(r.name)}">
        <div class="recommend-icon">${icon}</div>
        <div class="recommend-info">
          <h4>${esc(r.name.split('/')[1])}</h4>
          <p>${r.description ? esc(r.description.substring(0, 60)) + '...' : '-'}</p>
        </div>
        <div class="recommend-stats">
          <span>⭐ ${fmtNum(r.stars)}</span>
          <span>${esc(r.language || 'Unknown')}</span>
        </div>
      </div>
    `;
  }).join('');
