...
- 一级目录 = 技术领域（`ai-agents` / `ai-engineering` / `ai-mental-health` / `creative-coding` / `culture-arts` / `fullstack-arch` / `interview-career` / `maas-platform` / `mind-philosophy` / `mindfulness-apps`），注册于 `catalog.py` 的 `DOMAINS`
- 文件名 = 小写仓库名（`_` 转 `-`）+ `.md`
