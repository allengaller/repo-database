...
```mermaid
graph TB
A["index.html<br/>页面结构与可翻译节点"] --> B["app.js<br/>I18N 数据与函数"]
...
## 核心组件
- I18N 字典：以语言代码为键（如 zh、en），值为该语言下的所有文案键值对。
...
### I18N 对象与 t() 函数
- I18N 对象包含 zh 与 en 两套文案，键名统一且语义清晰，覆盖搜索、排序、统计、提示、模态框、灵感模式等场景。
- t(key) 通过 currentLang 索引 I18N，若找不到对应键则返回 key 作为降级显示，避免空白文案。
...
- [web/app.js:613-632](file://web/app.js#L613-L632)
- [web/app.js:670-716](file://web/app.js#L670-L716)
- [web/app.js:776-817](file://web/app.js#L776-L817)
- [web/app.js:2292-2399](file://web/app.js#L2292-L2399)
...
- app.js 定义 I18N、currentLang、t() 及语言切换函数，并通过事件监听绑定到按钮。
- localStorage 用于持久化语言偏好。
- 其他模块（如筛选、排序、渲染）在渲染时可能直接使用 currentLang 分支或 t()，形成间接依赖。

```mermaid
graph LR
HTML["index.html<br/>按钮与节点"] --> APP["app.js<br/>I18N/t/切换函数"]
...
## 结论
本项目的前端国际化方案简洁高效：以 I18N 字典为核心、t() 为统一访问入口、localStorage 持久化语言偏好，配合 applyTranslationsToUI() 集中刷新静态文案。为保证良好的可维护性与可扩展性，建议逐步将动态渲染中的硬编码文案迁移至 I18N，并通过 t() 注入，从而降低分支复杂度，提升新增语言时的可靠性与一致性。
