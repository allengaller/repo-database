...
## 更新摘要
**所做更改**
- 增强了网络请求基础设施，添加了全面的指数退避重试机制
- 统一了两个脚本中的评分算法，增加了更多指标维度
- 改进了数据合并逻辑，实现了字段级最大值操作和增强的来源跟踪
- 更新了错误处理和恢复策略
...
**更新** 系统现已集成强大的网络弹性机制，包括指数退避重试、统一评分算法和改进的数据合并逻辑。
...
**更新** 所有网络请求现在都通过统一的会话管理，具备指数退避重试机制，确保在网络不稳定情况下的稳定性。
...
**更新** 新增的网络弹性层确保所有外部 API 调用都具有自动重试和指数退避能力。
...
User->>Scrape : 触发完整采集
Scrape->>Retry : 创建带重试的会话
Retry->>GH : 搜索/详情/commit_activity (最多重试3次)
Retry->>HN : 拉取Top Stories并解析链接
Retry->>DEV : 拉取热门文章并解析链接
...
### 增强的网络弹性机制
**新增功能** 系统现在集成了全面的 HTTP 重试机制，使用指数退避算法处理临时网络错误。

- **实现要点**
  - 使用 `urllib3.util.retry.Retry` 配置重试策略
  - 针对 HTTP 状态码 429（限流）、500、502、503、504 自动重试
  - 最大重试次数：3次，退避因子：1.0秒（1s, 2s, 4s间隔）
  - 仅对 GET 方法启用重试，避免重复请求副作用
  - 通过 `HTTPAdapter` 挂载到 requests.Session
...
```mermaid
flowchart TD
Start(["HTTP 请求"]) --> Check{"状态码检查"}
Check --> |200| Success["成功响应"]
Check --> |429/500/502/503/504| Retry["指数退避重试"]
Check --> |其他错误| Fail["失败处理"]
Retry --> Wait["等待退避时间(1s→2s→4s)"]
Wait --> Check
Success --> End(["完成"])
Fail --> End
```
...
图表来源
- [scrape.py:114-130](file://scripts/scrape.py#L114-L130)

章节来源
- [scrape.py:114-130](file://scripts/scrape.py#L114-L130)
- [test_scripts.py:262-268](file://tests/test_scripts.py#L262-L268)
...
S->>Retry : 创建带重试的会话
Retry->>GH : 构造查询(时间/语言/主题)
GH-->>Retry : 返回 items (自动重试)
...
**更新** 改进的合并逻辑现在支持更精细的字段级操作，确保数据质量最优。
...
- [update_monthly.py:154-164](file://scripts/update_monthly.py#L154-L164)
...
**更新** 增量更新现在使用与主采集脚本相同的评分算法和数据合并逻辑，确保数据一致性。
...
**更新** 依赖简化为仅 requests，移除了不必要的 beautifulsoup4 依赖。
...
**更新** 新的网络弹性机制显著提高了系统在不可靠网络环境下的稳定性和成功率。
...
## 结论
该系统通过多源采集、严格标准化与去重、多维评分与增量更新，构建了高质量、可维护的仓库发现平台。配合前端交互与自动化流水线，实现了从数据到展示的闭环。

**更新** 最新的增强包括强大的网络弹性机制、统一的评分算法和改进的数据合并逻辑，进一步提升了系统的稳定性和数据质量。未来可在并发、缓存、更细粒度的评分权重调优等方面持续演进。
...
**更新** 依赖要求已简化，仅需 requests 包即可运行。

章节来源
- [README.md:43-75](file://README.md#L43-L75)
- [scrape.py:22-76](file://scripts/scrape.py#L22-L76)
- [update_monthly.py:300-343](file://scripts/update_monthly.py#L300-343)
